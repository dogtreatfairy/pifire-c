...
**Bluetooth Probes** offers *Connect*, *Edit* and *Delete* (the last two only once something is paired). *Connect* lists the makes (Chef iQ, MEATER, Inkbird / iBBQ), scans for eight seconds without blocking the panel, and shows what it found with signal bars; confirming one writes the device and its probes into settings exactly as the web pairing flow does, names them `BT1`, `BT2` and so on, and hides the ambient sensor from the home screens. *Edit* lists the paired probes with On or Off beside each and a press toggles one, so a probe can be parked without unpairing it. *Delete* asks for confirmation and then removes the probe and its ambient sibling together. All three work on the whole device, because a probe and its handle sensor are one piece of hardware.
