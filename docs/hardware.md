...
**Connect Bluetooth Probe** lists the makes (Chef iQ, MEATER, Inkbird / iBBQ), scans for eight seconds without blocking the panel, and shows what it found with signal bars. Confirming one writes the device and its probes into settings exactly as the web pairing flow does, names them `BT1`, `BT2` and so on, and hides the ambient sensor from the home screens.
