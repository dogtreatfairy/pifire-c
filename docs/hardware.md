...
While the grill is stopped the panel menu also offers **Network info** and **Power**. *Network info* draws a QR code for the grill's own web address (`http://<ip>/`, from the live network state in the status stream) with the address and the Wi-Fi network underneath, so a phone can join without anyone typing an IP; the encoder is built in (`display/qr.c`: byte mode, error-correction level L, versions 1-5, verified module-for-module against an independent encoder). *Power* asks for restart or shut down, starts on the harmless *Back* row, stops the grill first and then calls `systemctl`.
