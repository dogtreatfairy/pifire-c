...
The menu is a navigation stack: rows name modes and actions, never sentences, a *Back* row unwinds one screen and a long press returns to the grill from anywhere. What it offers follows the mode.

*Stopped* — **Startup** (which asks whether to run into Hold or Smoke), **Monitor**, **Network Info**, **Power**. Choosing *Startup To Hold* opens the temperature selector: the knob moves between the value, a *Startup* button on the bottom right and *Back* on the bottom left, clamping at the ends rather than wrapping, and pressing the value starts editing it so the knob changes the number instead.

*Cooking* (the active menu) — the opposite mode (**Hold Mode** from Smoke, **Smoke Mode** from Hold; Hold reuses the same temperature selector and applies at once, Smoke asks for confirmation), **End Cook** (a graceful shutdown), **Probe Target** (pick a probe, then the same selector with *Save*), **Connect Bluetooth Probe**, **Network Info** and **Emergency Stop**, which confirms on a red screen with *Cancel* on the left and *Stop* on the right.

*Monitor* — **Control**, **Startup**, **Stop**, **Connect Bluetooth Probe**, **Network Info**. *Control* shows the pit temperature above the auger, fan and igniter as rows the knob selects and a press toggles; leaving through *Exit* switches all three off again.

**Bluetooth Probes** offers *Connect*, *Edit* and *Delete* (the last two only once something is paired). *Connect* scans for eight seconds without blocking the panel, and shows what it found with signal bars; confirming one writes the device and its probes into settings exactly as the web pairing flow does, names them `BT1`, `BT2` and so on, and hides the ambient sensor from the home screens. *Edit* lists the paired probes with On or Off beside each and a press toggles one, so a probe can be parked without unpairing it. *Delete* asks for confirmation and then removes the probe and its ambient sibling together. All three work on the whole device, because a probe and its handle sensor are one piece of hardware.

**Network info** and **Power**. *Network info* draws a QR code for the grill's own web address (`http://<ip>/`, from the live network state in the status stream) with the address and the Wi-Fi network underneath, so a phone can join without anyone typing an IP; the encoder is built in (`display/qr.c`: byte mode, error-correction level L, versions 1-5, verified module-for-module against an independent encoder). *Power* asks for restart or shut down, starts on the harmless *Back* row, stops the grill first and then calls `systemctl`.

### Bluetooth probes

`ibbq` (Inkbird / iBBQ 4- and 6-probe), `meater` (MEATER Original/Plus and Pro) connect over GATT. `chefiq` (Chef iQ CQ50 / CQ60) is **passive**: the probe broadcasts its readings in manufacturer-specific advertisements (company id 0x05CD) and the daemon keeps LE discovery running to read them — no pairing, no base station, and the phone app can stay connected at the same time. Pair it from the Probes page → *Add Probe*, which scans straight away and lists it as `CQ60` with its make beside it — the scan classifies every device it sees (company id 0x05CD, the MEATER service UUIDs, an iBBQ name or the `0000fff0` service) and that is what chooses the module, so the make is never asked for. *Choose the make myself* under the scan is the way in for a probe the classifier does not recognise; leaving the address empty adopts the first Chef iQ found. Ports: `BT_Probe` (the internal / meat sensor) and `BT_Ambient` (the handle sensor). Pairing creates both as probes of the same device; the ambient one is shown as a second readout inside the probe's card on Home and on the grill display rather than as a separate card. Battery arrives in the probe's status packet. Wire format after the `chefiq-ble` library used by Home Assistant (V3 layout verified on a CQ60; V2/legacy layouts implemented as documented but unverified).

**Chef iQ in practice (measured on a CQ60 with BlueZ 5.66).** The probe advertises about every 4 s only while it is out of its dock and awake; in the dock it is silent, so it cannot be paired or read until it is taken out. Its temperature packet is byte-identical while the temperature is steady, and BlueZ then raises no property change at all (even with the DuplicateData filter), so the daemon re-reads BlueZ's cached payload every second while the device object exists; BlueZ drops a device it has not heard for ~30 s, which is what ends the readings. The pairing dialog lists only devices recognised as Chef iQ probes (company id 0x05CD, ≤ 18-byte payload); the Chef iQ hub ("Smart Thermometer", 21-byte payload) is never offered. Removing either sensor unpairs the whole probe, and a wireless device left without readings is dropped from settings so its address is offered again.
