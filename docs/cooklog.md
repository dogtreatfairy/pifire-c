...
The adaptive controller's PB/Ti/Td in force are in `controller.tuning_note`; the events show when they changed (`TUNING_LEARNED`). Manual tuning goes in *Settings → Cooking → Hold Mode → Controller*; turning off *Learn from cooks* freezes the loop at the entered values.
